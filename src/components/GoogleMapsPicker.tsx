...
const GoogleMapsPicker: React.FC<GoogleMapsPickerProps> = ({
  ...
  const initMap = () => {
    ...
    if (!google.maps.places) {
      ...
      console.error(
        "Google Maps Places library failed to load. Please refresh the page."
      );
      return;
    }
    ...
    const handlePositionSuccess = (position: GeolocationPosition) => {
      ...
      if (markerRef.current && googleMapRef.current) {
        ...
        if (position.coords.accuracy) {
          ...
          accuracyText.innerHTML = `Location accuracy: ${Math.round(
            accuracy
          )}m`;

          googleMapRef.current.controls[
  ...
  return (
    <div className="space-y-4">
      ...
      <div ref={mapRef} className="h-96 w-full rounded border"></div>

      {coordinates.lat !== 0 && coordinates.lng !== 0 && (
        <div className="text-sm text-gray-500">
          Selected coordinates: {coordinates.lat.toFixed(6)},{" "}
